...
const utils = {
  ...
  loadUserConfig: async (checkIsEmpty = true) => {
    ...
    if (config.apiToken) {
      ...
      if (decoded.payload && decoded.payload.iss) {
        if (decoded.payload.iss === 'api.getstructure.io') {
          decoded.payload.iss = 'api.app.wnology.io';
        }
    ...
    if (checkIsEmpty && isEmpty(config)) {
      utils.logError('User Configuration file missing, run wegnology login to generate this file.');
      process.exit(1);
    }
    return config;
  },
  ...
  loadApplicationConfig: async (file, userConfig) => {
    ...
    if (!foundFile) {
      utils.logError('Configuration file missing for this directory, run wegnology configure to generate this file.');
      return;
    }
  ...
  loadLocalMeta: async (type) => {
    const dir = path.resolve('.wegnology');
  ...
  getLocalStatus: async (type, dir, globPattern) => {
    ...
    const dirPattern = path.resolve(path.join(dir, globPattern));
    const files = globSync(dirPattern.split(path.sep).join(path.posix.sep)); // glob sync always wants slashes the same way as linux even on windows
  ...
  isValidExperienceOptions: (cmdOpts) => {
    if (!cmdOpts.type) { return true; }
    const newType = utils.plural(cmdOpts.type);
    if (!(/^(layouts|components|pages)$/i).test(newType)) {
      utils.logError(`Invalid Type: ${cmdOpts.type}, only valid views types are layouts, pages and components.`);
      return false;
    }
    cmdOpts.type = newType;
    return true;
  },
  hasBootstrapped: (application) => {
    const result = find(propEq('experience', 'name'), propOr([], 'ftueTracking', application));
