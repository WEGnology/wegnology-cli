...
module.exports = async ({ url, email, password, twoFactorCode, apiToken } = {}) => {
  ...
  if (email) {
    ...
    const results = await api.auth.authenticateUser({ credentials: { email: trim(email), password, twoFactorCode, requestedScopes: [ 'all.Application', 'applications.get' ], tokenTTL: 0 } });
